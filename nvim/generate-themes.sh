#!/bin/bash
# Generate neovim colorschemes from ricebox theme files

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
THEME_DIR="${1:-$SCRIPT_DIR/../themes}"
OUT_DIR="$SCRIPT_DIR/colors"
...
for theme_file in "$THEME_DIR"/*.sh; do
  ...
  cat > "$OUT_DIR/ricebox-${name}.lua" << LUAEOF
-- ricebox-${name}
-- auto-generated from ricebox theme: ${name}

vim.cmd("hi clear")
if vim.fn.exists("syntax_on") then vim.cmd("syntax reset") end
vim.o.termguicolors = true
vim.g.colors_name = "ricebox-${name}"
...
local c = {
  ...
  blue_br  = "${BLUE_BRIGHT}",
  magenta  = "${MAGENTA}",
  magenta_br="${MAGENTA_BRIGHT}",
  cyan     = "${CYAN}",
  cyan_br  = "${CYAN_BRIGHT}",
  white    = "${WHITE}",
  white_br = "${WHITE_BRIGHT}",
}
...
LUAEOF
  ...
  echo "  generated ricebox-${name}.lua"
done

echo "done - $(ls "$OUT_DIR"/*.lua | wc -l) themes generated"
